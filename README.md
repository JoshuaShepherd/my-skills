...
| `~/.claude/skills/<name>/` (real bundle, not under `~/.agents`) | `<name>/` |
...
Some projects store a single folder `.claude/skills/skills-anthropic/` whose **`SKILL.md` files live under `skills/<name>/`**, not at the bundle root. The sync script does **not** split those into top-level `<name>/` folders.
...
1. Pick a bundle path: `<name>/`, `cursor/<name>/`, `agents/<name>/`, or `repo-specific/<portal>/<name>/`.
2. Copy or symlink it into the matching directory on your machine.
...
ln -sf "$REPO_ROOT/article-author" ~/.claude/skills/article-author
...
Use **`SKILLS_MANIFEST.json`** as the machine-readable index (`skill_count`, canonical paths, alternates).
