...
1. `README.md` — dashboard + executive summary (highest-signal, almost always present).
2. `summary.md` / `<SLUG_UPPER>_RESEARCH_SUMMARY.md` — digital presence facts, current role, books list, podcast counts.
3. `biography.md` (when present — e.g., Alan Hirsch) — narrative bio + voice/character notes.
4. `<SLUG_UPPER>_AUTHOR_PROFILE.md`, `<SLUG_UPPER>_COMPLETE_PROFILE.md` — structured author dossier.
5. `<SLUG_UPPER>_ORGS.md` — org list (drives `credentials` + `secondary_links`).
6. `<SLUG_UPPER>_VOICE_IDENTITY.md` — voice markers, signature frameworks, themes.
7. `<SLUG_UPPER>_CALLING_PROFILE.md` — vocational arc (informs the second bio paragraph).
8. `fragmentation-story.md` — motivation framing (informs paragraph three's hinge).
9. `identity-verification.md` — disambiguation hard facts (location, birth year, name spelling).
10. `digital-presence-discovery.md` — verified URLs, social handles.
11. `content-analysis.md` / `content/books.md` — book titles, publishers, years.
...
- **location_line** — "City, State" (US) or "Region & Region" (multi-base). Pull from `identity-verification.md` or `README.md` Quick Facts. Examples: "Los Angeles area, California", "Colorado Springs, Colorado", "Central Florida & Puerto Rico".
...
### Themes
- **themes** — exactly three Title Case phrases, each ≤ 28 chars. These are the leader's load-bearing surfaces, not generic categories. Pull from `<SLUG_UPPER>_VOICE_IDENTITY.md` "frameworks and themes" section when present, else from the dossier's executive summary. Prefer specificity ("Mujerista theology") over generality ("Theology"). Don't use "Leadership" or "Discipleship" alone — too generic.
...
### Links
- **primary_url** — the leader's canonical personal/author domain. From `digital-presence-discovery.md` or `README.md`. Absolute URL.
...
- **signature_frameworks** — 3 to 7 entries of `{ name, definition }`. The leader's load-bearing frameworks. Definition is one sentence. Pull from `<SLUG_UPPER>_VOICE_IDENTITY.md` "frameworks and themes" or `biography.md` "Key Contributions" when present.
- **lineage** — optional `{ sources, peers, downstream }` — three to ten entries each, slugs preferred where the named person is also a movement leader in the network. Drawn from `network/collaborators.md` or `<SLUG_UPPER>_ORGS.md` if present; otherwise omit entirely.
...
contains some subset of: README dashboard, executive summary,
biography, author profile, calling profile, orgs list, voice identity,
fragmentation story, content audit, digital presence inventory.
