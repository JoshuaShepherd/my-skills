...
- What video to create (course video, social clip, trailer, presentation, animated explainer)
...
**Time math:** `seconds × fps = frames`. So 5 seconds at 30fps = 150 frames.
...
<Audio
  ...
  volume={(f) =>
    // Fade in over 1 second, then full volume
...
// Embed a Veo/Sora generated clip
<OffthreadVideo
...
export const CourseIntro: React.FC = () => {
  return (
    <AbsoluteFill>
      {/* Background video from Veo/Sora */}
...
export const calculateMetadata: CalculateMetadataFunction<Props> = async ({ props }) => {
  // Fetch audio duration to set video length
...
### TTS → Remotion Pipeline

1. Generate narration with **OpenAI TTS** or **ElevenLabs** skill
2. Save audio files to `public/` directory of Remotion project
3. Use `<Audio src={staticFile("narration.mp3")} />` in composition
4. Use `getAudioDurationInSeconds()` to match video duration to audio

### Veo/Sora → Remotion Pipeline

1. Generate video clips with **Veo 3.1** or **Sora 2** skill
2. Save clips to `public/` directory
3. Use `<OffthreadVideo src={staticFile("clip.mp4")} />` in composition
4. Layer text overlays, transitions, and audio on top

### Nano Banana 2 → Remotion Pipeline
...
|---|---|---|
| Free (personal) | $0 | Non-commercial, learning |
| Company | From $0 (revenue-based) | Revenue < $25K/yr |
| Enterprise | Custom | Large scale, Lambda |
...
### Assets
- Audio: public/narration.mp3 (TTS via OpenAI)
- Video: public/hero-clip.mp4 (Veo 3.1)
- Images: public/hero-image.png (Nano Banana 2)
...
- Use `AbsoluteFill` as the root container — it's `position: absolute; inset: 0`
...
- Use `OffthreadVideo` (not `Video`) — it processes video in a separate thread for better performance
