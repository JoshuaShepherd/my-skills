...
`$ARGUMENTS` — Optional output directory path. Defaults to `{{OUTPUT_DIR}}`.

If $ARGUMENTS is provided, use it as the output directory. Otherwise use `{{OUTPUT_DIR}}`.
...
| 05 | `05-COMPONENT-LIBRARY.md` | Available UI components (shared library + custom), their variants, when to use each. Component hierarchy and composition patterns. |
...
| 16 | `16-ANIMATION-MOTION.md` | Motion design philosophy, animation libraries, scroll animations, micro-interactions, performance budget. |
...
| 18 | `18-INTEGRATIONS.md` | Third-party services and their roles, data flow, webhook patterns. Discovered from project dependencies and configuration. |
