...
description: Create or update the author visual style guide that ensures consistent lighting, color grade, and composition across all author imagery — trigger phrases include "author style guide", "visual style for author", "create style guide", "update style guide", or "ensure image consistency".
---
...
`{{ASSET_DOCS_DIR}}/author-style-guide.md`. Every other asset skill (`/asset-headshot`,
...
1. Read `{{CONFIG_PATH}}` for brand identity and existing image URLs
2. Check if `{{ASSET_DOCS_DIR}}/author-style-guide.md` already exists
...
that can be pasted directly into Gemini prompts:
...
### Physical Description (for Gemini prompt consistency)
[A precise, respectful physical description of the author as they appear in the
reference photos. This ensures Gemini maintains likeness when extending backgrounds
...
| Background lights | Amber, honey, cream | {{BRAND_PRIMARY}} to {{BRAND_LIGHT}} range |
| Background darks | Warm charcoal, espresso | {{BRAND_DARK}} to #3D3D3D range |
...
> warm charcoal {{BRAND_DARK}}, not pure black).
...
```
[Paste the physical description from Section 1. This ensures Gemini maintains consistency
...
every description should be precise enough to paste directly into an Gemini prompt.
...
Write to `{{ASSET_DOCS_DIR}}/author-style-guide.md`.
...
### Author: {{AUTHOR_NAME}}
### File: {{ASSET_DOCS_DIR}}/author-style-guide.md
...
### Test Generations
1. Headshot (1:1): {{OUTPUT_DIR}}/style-test/alan-headshot-test.png ✅
2. Hero (16:9): {{OUTPUT_DIR}}/style-test/alan-hero-test.png ✅
...
```
## Author Style Guide — {{AUTHOR_NAME}}
...
1. `BLOCK: Subject Likeness` — physical description for Gemini consistency
...
### Image: {{OUTPUT_DIR}}/headshot/alan-instructor.png
### Style Guide: {{ASSET_DOCS_DIR}}/author-style-guide.md
...
2. Lift the blacks — darkest point should be {{BRAND_DARK}}, not pure black
