...
- Auth pages at `src/app/(public)/auth/signup/`
...
Search the codebase for:
- Supabase service role key in client-side code (NEVER should be in `NEXT_PUBLIC_*`)
...
grep -r "SUPABASE_SERVICE_ROLE" src/ --include="*.ts" --include="*.tsx" -l
```

```bash
# Check env validation
grep -r "NEXT_PUBLIC_SUPABASE" src/lib/env.ts
```

**Flag as CRITICAL if:** Service role key appears in any `NEXT_PUBLIC_*` variable or client-side code.
...
```
src/app/api/ — check for routes that don't call supabase.auth.getUser()
```
