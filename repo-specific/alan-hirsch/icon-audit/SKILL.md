---
name: icon-audit
description: Audit and fix icon and illustration usage across the codebase — mixed libraries, wrong sizes, hardcoded colors, missing accessibility, flex compression, and illustration anti-patterns. Run before any design review.
user-invocable: true
...
1. Read `src/app/globals.css` to confirm the current semantic color token inventory.
2. Read `src/components/ui/icon.tsx` if it exists — this is the canonical Icon wrapper. If it doesn't exist, note it as a critical gap.
3. Check which icon libraries are installed: `grep -r "from 'lucide-react'\|from '@heroicons\|from 'react-icons\|from 'phosphor" package.json`

## Scan Checklist

### 1. MIXED ICON LIBRARIES (Critical)

This codebase uses **Lucide React only**. Any other icon library is a violation.

Search for:
- `from '@heroicons/react'`
- `from 'react-icons'`
- `from 'phosphor-react'`
- `from '@radix-ui/react-icons'` — allowed ONLY inside `src/components/ui/*` (shadcn internals)
- `from 'feather-icons'`
- Any `<svg>` with inline path data that duplicates a Lucide icon

**Fix:** Replace with the equivalent Lucide icon. Map using the Lucide icon search at lucide.dev.

### 2. RAW LUCIDE USAGE (High)

Icons should always be used via the canonical `<Icon>` wrapper (`src/components/ui/icon.tsx`), not imported and rendered directly. Raw usage bypasses the size scale, accessibility defaults, and flex-compression guard.
...
**Allowed exception:** Inside `src/components/ui/*` (shadcn primitives) and the Icon wrapper itself.
...
- Raster PNGs in `public/images/illustrations/` that could be SVG
...
Lucide icons default to `strokeWidth={2}`. At `xl` / `2xl` sizes used in marketing sections, `strokeWidth={1.5}` looks more refined. Never mix 1.5 and 2 in the same UI region.
...
3. **Never modify `src/components/ui/*`** (shadcn internals).
...
[ ] src/components/ui/icon.tsx does not exist → Run /icon-system to create it
...
#### 2. RAW LUCIDE USAGE — X violations
| File | Line | Violation | Fix |
|------|------|-----------|-----|
