---
name: asset-video-prompt
description: Generate video transition prompts for AI video models (Runway, Kling, Pika, Higgsfield). Creates start/end frame descriptions, transition choreography, and model-agnostic instructions. Use for scroll-stopping social content, product reveals, course trailers, or any motion asset.
user-invocable: true
...
1. Read `src/lib/config/tenant.config.ts` for brand context
...
## Brand-Aligned Video Concepts for Alan Hirsch Platform
...
If the user doesn't have start/end images yet, generate NB2 image prompts first:
...
### START FRAME PROMPT
[For image generation — paste into NB2/Midjourney/etc.]
...
[Paste into your video model, upload start frame as first frame, end frame as last frame]

{full 5-section video prompt}
...
| Model | Any (Runway, Kling, Pika, Higgsfield) |

### Workflow
1. Generate start frame image with `/asset-generate` using the start frame prompt
2. Generate end frame image with `/asset-generate` using the end frame prompt
3. Upload both to your video model with the transition prompt
4. Use `/asset-brand-check` on the frames before generating video
```
...
| Issue | Fix |
|-------|-----|
| Video model produces jittery motion | Add "smooth, eased motion, no sudden jumps" and reduce complexity |
| Start/end frames look inconsistent | Regenerate end frame using start frame as reference in NB2 |
| Motion too fast | Increase duration, add "slow, deliberate, contemplative pace" |
| Background changes during transition | Add "background remains static and unchanged throughout" |
| Artifacts on moving elements | Simplify the transition — fewer moving parts, one clear axis of motion |
| Camera drifts | Add "locked-off tripod shot, absolutely no camera movement" |
