...
1. **Read the target page's current implementation** (if it exists) to understand what hooks, data sources, and patterns are already in use.
2. **Read `src/app/globals.css`** to understand current design tokens.
3. **Read `src/lib/config/tenant.config.ts`** to understand tenant theming and feature flags.
4. **Read the Stitch screen** via `mcp__stitch__get_screen` to get the full HTML/CSS.
5. **Identify which database entities the page needs** (e.g., homepage needs pathways, content items, courses). Map each to its Layer 5 hook file in `src/hooks/simplified/` or `src/hooks/custom/`.
