---
name: studio-design
description: Generate a rich AI Studio design prompt for any mock-up or prototype — pages, dashboards, heroes, apps. Combines AntiGravity 5-dimension framework, 67+ styles, template patterns, and evocative design language so AI Studio makes beautiful design decisions autonomously.
user-invocable: true
...
- **Warm & Scholarly** — Amethyst, sage, cream, warm charcoal. Inviting, intellectual.
...
---
*Paste this prompt into AI Studio Build mode. The brief is intentionally directional — not prescriptive — so the AI can make beautiful design decisions within these guardrails.*
```

Save to: `_docs/studio-designs/[feature-slug].md`

Create the `_docs/studio-designs/` directory if it doesn't exist.

## Generation Rules

1. **Be evocative, not prescriptive.** "Warm amber tones that feel like afternoon sunlight on old paper" > "#F59E0B background".
2. **Describe experiences, not implementations.** "Cards lift on hover like they're being picked up" > "translateY(-4px) with box-shadow".
3. **Give direction, not dictation.** Provide enough guardrails that the result is on-brand, but enough freedom that the AI can surprise you.
4. **Be specific about structure, poetic about style.** The skeleton (layout/pattern) should be clear; the skin (style/color/type/motion) should be inspirational.
5. **Always include anti-patterns.** Telling the AI what NOT to do is as important as telling it what TO do.
6. **Use the template library as a vocabulary.** Reference hero variants, page patterns, and section types from the template library when relevant — these are proven patterns.
7. **Never include code.** No CSS, no HTML, no Tailwind classes. Pure design language.
8. **Keep it to one page.** The prompt should be comprehensive but scannable — AI Studio works best with focused, clear briefs.
