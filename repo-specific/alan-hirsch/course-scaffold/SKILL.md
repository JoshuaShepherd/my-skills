...
5. Read `scripts/ingest-mdna-course.ts` for insert patterns
...
2. Creates the `courses` row with:
   ...
   - pathway_themes: `["<pathway>"]` if pathway provided
...
- Follow the project's postgres insert patterns from `scripts/ingest-mdna-course.ts`
