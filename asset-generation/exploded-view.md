...
Generate an exploded/deconstructed view using Gemini Image Generation: $ARGUMENTS
...
#### 5Q / APEST Framework ({{AUTHOR_NAME}})
...
for (const part of response.candidates[0].content.parts) {
  if (part.inlineData) {
    ...
    const outPath = path.join("{{OUTPUT_DIR}}/exploded", `${slug}-exploded.png`);
...
### Generated Files
1. {{OUTPUT_DIR}}/exploded/{{course-slug}}-book-exploded.png
