...
2. Read `{{CONFIG_PATH}}` — check existing `hero.imageUrl` and `courseInfo.instructorHeadshotUrl`
3. Read the source image to understand current quality, lighting, background, and expression
4. Check if an author style guide exists at `{{ASSET_DOCS_DIR}}/author-style-guide.md` (created by `/asset-author-style`) — if so, follow it for consistency
...
## Current Author Images ({{BRAND_NAME}} Platform)
...
### 2. Warm Bokeh (Default for {{AUTHOR_NAME}})
...
({{BRAND_PRIMARY}}) at the upper left to deep warm charcoal ({{BRAND_DARK}}) at the lower right. Subtle
...
for (const part of response.candidates[0].content.parts) {
  if (part.inlineData) {
    ...
    const outPath = path.join("{{OUTPUT_DIR}}/headshot", `${slug}-${targetUse}.png`);
...
After Gemini generation, resize to exact pixel dimensions using sharp:
...
for (const [name, dims] of Object.entries(sizes)) {
  await sharp(highResSource)
    ...
    .webp({ quality: 90 })
    .toFile(`{{IMAGES_DIR}}/headshots/{{author-slug}}-${name}.webp`);
}
```
...
| Use | File | Dimensions |
|-----|------|-----------|
| instructor | {{IMAGES_DIR}}/headshots/{{author-slug}}-instructor.webp | 120×120 |
| bio-card | {{IMAGES_DIR}}/headshots/{{author-slug}}-bio-card.webp | 300×300 |
| avatar-lg | {{IMAGES_DIR}}/headshots/{{author-slug}}-avatar-lg.webp | 256×256 |
...
| Issue | Fix |
|-------|-----|
| Face distorted by Gemini | Reduce edit scope — do background-only first, then lighting as separate pass |
