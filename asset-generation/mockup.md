...
3. Read `{{CONFIG_PATH}}` for brand context
...
### Method A — Gemini Generation (Recommended)
Use Gemini Image Generation to generate the entire mockup scene with the content described in the prompt.
...
for (const part of response.candidates[0].content.parts) {
  if (part.inlineData) {
    ...
    const outPath = path.join("{{OUTPUT_DIR}}/mockup", `${slug}-mockup.png`);
...
// Templates stored in {{IMAGES_DIR}}/mockup-frames/
...
For the {{AUTHOR_NAME}} platform, default to these scene styles:
...
### Generated Files
1. {{OUTPUT_DIR}}/mockup/{{course-slug}}-laptop-mockup.png
