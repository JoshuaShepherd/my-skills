...
Generate a consistent asset series using Gemini Image Generation: $ARGUMENTS

$ARGUMENTS should include:
- The series concept (e.g., "8 course module covers for The {{COURSE_NAME}}")
...
2. Read `{{CONFIG_PATH}}` for brand context
3. Read `{{STYLES_PATH}}` for design tokens
...
Gemini supports up to 5 character consistency and 14 object tracking. Use these features to maintain visual continuity:
...
- Color palette: warm earth tones — terracotta ({{BRAND_ACCENT}}), sage ({{BRAND_SECONDARY}}), amber ({{BRAND_PRIMARY}}), cream ({{BRAND_LIGHT}})
...
for (const part of firstResult.candidates[0].content.parts) {
  if (part.inlineData) {
    ...
    fs.mkdirSync("{{OUTPUT_DIR}}/series/<series-slug>", { recursive: true });
    fs.writeFileSync("{{OUTPUT_DIR}}/series/<series-slug>/01-<item-slug>.png", buffer);
  }
}
```
...
for (let i = 1; i < items.length; i++) {
  ...
  for (const part of response.candidates[0].content.parts) {
    if (part.inlineData) {
      ...
      fs.writeFileSync(
        `{{OUTPUT_DIR}}/series/<series-slug>/${num}-${items[i].slug}.png`,
        buffer
      );
    }
  }
...
### Series: The {{COURSE_NAME}} — Course Module Covers
### Items: 8 modules
...
| # | Title | File | Status |
|---|-------|------|--------|
| 01 | Missional Renaissance | {{OUTPUT_DIR}}/series/{{course-slug}}/01-missional-renaissance.png | ✅ |
| 02 | The DNA of Movement | {{OUTPUT_DIR}}/series/{{course-slug}}/02-dna-of-movement.png | ✅ |
| 03 | Apostolic Genius | {{OUTPUT_DIR}}/series/{{course-slug}}/03-apostolic-genius.png | ✅ |
| ... | ... | ... | ... |
