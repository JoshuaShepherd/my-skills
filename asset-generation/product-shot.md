...
Generate a product photography shot using Gemini Image Generation: $ARGUMENTS
...
- Brand-appropriate styling (warm earth tones for {{AUTHOR_NAME}})
...
for (const part of response.candidates[0].content.parts) {
  if (part.inlineData) {
    ...
    const outPath = path.join("{{OUTPUT_DIR}}/product", `${slug}.png`);
...
### Generated Files
1. {{OUTPUT_DIR}}/product/{{course-slug}}-book-1.png
