...
Generate an image asset using Gemini Image Generation: $ARGUMENTS
...
2. Read `{{CONFIG_PATH}}` to understand the tenant brand context
3. Read `{{STYLES_PATH}}` to understand the color palette and design tokens
...
Structure every prompt using this 5-part brief. Write in natural, descriptive language — NOT keyword soup. Gemini is a reasoning model that understands intent, physics, and composition.
...
## Brand Context for {{BRAND_NAME}} Platform
...
Write the full Gemini prompt following the brief method above. Show it to the user for approval before generating.
...
for (let i = 0; i < result.generatedImages.length; i++) {
  ...
  const outPath = path.join("{{IMAGES_DIR}}/generated", `${slug}-${i + 1}.png`);
...
### Step 3 — Save & Report
- Save images to `{{OUTPUT_DIR}}/<asset-type>/<slug>-<n>.png`
...
### Generated Files
1. {{OUTPUT_DIR}}/hero/{{course-slug}}-1.png
2. {{OUTPUT_DIR}}/hero/{{course-slug}}-2.png
3. {{OUTPUT_DIR}}/hero/{{course-slug}}-3.png
4. {{OUTPUT_DIR}}/hero/{{course-slug}}-4.png
...
If the first generation is close but not right, use Gemini's conversational editing:
