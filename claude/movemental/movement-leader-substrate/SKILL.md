...
2–3 paragraphs. Pull from `_misc/committed-voice.md` if present. Should answer:
...
If the per-topic source files exist (`profile/`, `content/`, `network/`, `analysis/`, `digital-presence/`, plus helper dirs `_misc/` and `_staff/`), this is `--refresh` mode: read those files and assemble the substrate.

If only a `_misc/committed-voice.md` exists, this is a thin profile — flag what's missing and offer to run targeted research.
...
|-------------------|--------------|
| Identity | `profile/identity.md`, `_staff/identity-verification.md`, `_misc/committed-voice.md` |
| Disambiguation | `profile/identity.md` (Disambiguation section), `_staff/identity-verification.md` |
| Editorial bio | `_misc/committed-voice.md` editorial bio block, `profile/biography.md` |
| Timeline | `profile/biography.md` (Career & Ministry Arc), `profile/identity.md` (Career Timeline) |
| Frameworks | `content/frameworks.md`, `profile/theology.md` (Key Frameworks & Models), content files for adoption evidence |
| Bibliography | `content/books.md` |
| Theme map + Content analysis | `content/content-audit.md`, `_misc/content-analysis-root.md` |
| Voice fingerprint | `profile/voice-analysis.md`, `_misc/committed-voice.md` signature_frameworks |
| Theological positioning | `profile/theology.md`, `_misc/content-analysis-root.md` (theological positioning) |
| Intellectual genealogy | `_misc/network/collaborators.md`, `profile/theology.md` (Intellectual Genealogy) |
| Network graph | `_misc/network/collaborators.md`, `_misc/network/endorsements.md`, `network/organizations.md` |
| Reach metrics | `analysis/audience-analysis.md`, `digital-presence/*`, `content/*`, `_misc/media/*` |
| Distribution inventory | `digital-presence/*`, `content/*`, `fragmentation-story.md` (factual inventory only) |
| Sources | `sources.md` |

> The lowercase categorized files (`profile/`, `content/`, `network/organizations.md`, `analysis/`, `digital-presence/`) are the canonical per-leader research surface — the same set present in the reference dossiers [`brad-brisco/`](docs/movement_leader_research/brad-brisco/) and [`michael-cooper/`](docs/movement_leader_research/michael-cooper/). Older ALL-CAPS profiles (`{SLUG}_AUTHOR_PROFILE.md`, `{SLUG}_VOICE_IDENTITY.md`, `{SLUG}_CALLING_PROFILE.md`, `{SLUG}_CONTENT_AUDIT.md`, `{SLUG}_AUDIENCE_PROFILE.md`, `{SLUG}_ORGS.md`, `{SLUG}_TIMELINE.md`, `{SLUG}_RESEARCH_SUMMARY.md`) only survive under `_staff/legacy/` and are fallbacks, not primary sources — a freshly researched leader will not have them.
