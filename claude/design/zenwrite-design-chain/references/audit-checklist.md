# ZenWrite Audit Checklist & Severity

Use this when auditing an existing page/component for design-chain alignment. Pair it with
`scripts/audit-scan.sh` (mechanical hits) — this file adds judgment and severity.
...
- [ ] Sphere discipline — content screens lead with violet; community screens lead with sky/emerald/rose; neither borrows the other's primary accent.
...
- `brand-violet #14006a` on dark surfaces fails — swap to `dark:text-indigo-300` (or lighter violet token).
- Every control reachable and operable by keyboard; visible focus ring on tab.
- Icon-only buttons need `aria-label`; decorative icons `aria-hidden`.
- Respect reduced motion where animations are non-essential.

## Responsive

- Verify mobile (single-column home, collapsed spine, bottom-nav-first), `md+` (dual-column, spine in book mode), and wide (`max-w-7xl` cap).
...
3. Apply fixes lowest-risk first (token swaps, focus rings) → structural (primitive extraction, z-index).
4. Never introduce a new hex or new color token to fix a hit — reuse existing tokens; only add to `@theme` if a genuinely new semantic color is required (rare; flag it).
5. Validate: `pnpm build:check` (and `RUN_BUILD_VALIDATION=true pnpm build:check` for the Vite bundle). Fix TS errors in `reports/tsc.txt`.
