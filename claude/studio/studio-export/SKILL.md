...
1. Identify the feature's data footprint:
   - **Schema tables** — `src/lib/database/schema.ts` (grep for relevant table names)
   - **Zod schemas** — `src/lib/schemas/` (find matching schema files)
   - **Services** — `src/lib/services/simplified/` and `src/lib/services/custom/`
   - **API routes** — `src/app/api/simplified/` and `src/app/api/custom/`
2. Read the page/component files in `src/app/(public)/` to understand the user-facing behavior
3. Read relevant `_docs/` files for feature narrative context
4. Read `src/lib/config/tenant.config.ts` for domain context and feature flags
...
**Rules:**
- Include the actual Zod validation shapes from `src/lib/schemas/`
...
Save the document to: `_docs/studio-exports/[feature-slug].md`
...
### Document
Saved to: `_docs/studio-exports/[feature-slug].md`
- Total sections: 5
