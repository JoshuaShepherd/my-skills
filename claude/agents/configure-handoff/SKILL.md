...
description: Configure agent handoffs — use when asked to set up multi-agent routing, delegate conversations between specialist agents, configure handoff rules, or wire agents together via an agent registry.
---
...
2. Read the existing handoff infrastructure:
   - `{{AGENTS_DIR}}/shared/handoffs.ts` — `AgentRegistry`, `HandoffDecision`, `HandoffHandler`
   - `{{AGENTS_DIR}}/shared/enhanced-agent-bridge.ts` — search for `enableHandoffs` and handoff event handling
   - `{{AGENTS_DIR}}/shared/agent-tracer.ts` — handoff event tracing
3. Read the database schema:
   - `{{SCHEMA_PATH}}` — search for `agent_handoffs` table (from_agent_id, to_agent_id, trigger_rules, priority)
