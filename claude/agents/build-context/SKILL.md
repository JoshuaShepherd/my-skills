---
name: build-context
description: Work on agent context — use when asked to build, modify, or debug the user context system for an agent, including context schemas, RunContract mapping, context block rendering, or catalog slug validation.
---
...
1. Read the context system across repos:
   - `{{PROJECT_ROOT}}/{{AGENTS_DIR}}/ai-lab/instructions/context.ts` — dynamic context section built per run
   - `{{PROJECT_ROOT}}/src/lib/chat/build-user-context-instructions.ts` — renders human-readable blocks from user profile
...
- 14 books (5Q, Reframation, The {{COURSE_NAME}}, etc.)
