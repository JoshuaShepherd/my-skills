---
name: add-tool
description: Create or modify an agent tool — use when asked to add a new capability to an agent, build a search/retrieval/analysis/API integration tool, or wire a tool into an existing agent.
---
...
2. Read existing tool implementations:
   ...
   - `{{PROJECT_ROOT}}/{{AGENTS_DIR}}/seo-expert/tools.ts` — 9-tool suite (analyze, research, score, optimize, generate)
...
3. Read the shared tool infrastructure:
   - `{{PROJECT_ROOT}}/src/services/toolCacheService.ts` — param-based LRU caching with TTL
