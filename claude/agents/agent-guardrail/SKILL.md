...
description: Add input/output guardrails to an agent pipeline — validation, content filtering, safety rules. Use when adding safety or quality gates to agents.
user-invocable: true
...
2. Read the existing guardrail implementation:
   - `src/agents/shared/guardrails.ts` — nonEmpty, maxLength, contentFilter, noErrors implementations
   - `src/agents/shared/enhanced-agent-bridge.ts` — `applyInputGuardrails()` and `applyOutputGuardrails()` in the bridge pipeline
3. Read the database schema for guardrail persistence:
   - `src/lib/database/schema.ts` — search for `agent_guardrails` and `agent_guardrail_assignments` tables
