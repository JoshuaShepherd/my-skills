...
description: Manage reusable NB2 prompt templates for recurring asset types. Use to store, retrieve, list, or customize prompt templates for consistent asset generation.
user-invocable: true
allowed-tools: Read, Write, Edit, Bash, Grep, Glob
---

Manage the NB2 prompt template library: $ARGUMENTS
...
Templates are stored in `_docs/asset-prompts/` as individual markdown files:
...
asset_type: <preset from asset-generate>
aspect_ratio: <default aspect ratio>
resolution: <default resolution>
thinking_mode: <minimal|high|dynamic>
variables:
  - name: title
  ...
  - name: subtitle
    ...
    default: ""
  - name: subject
    description: "The visual subject/motif"
    required: true
  - name: mood
    description: "Override the default mood"
    required: false
    default: "warm, scholarly, contemplative"
---
...
/asset-prompt-library use hero-course --var title="The Forgotten Ways" --var subject="an open compass on a hand-drawn map with a seedling growing beside it"
```
...
- Variables: `quote` (required), `attribution` (default: "Alan Hirsch"), `background`
...
- Variables: `title` (required), `author` (default: "Alan Hirsch"), `subject` (required), `genre_feel`
...
### `list`
Read `_docs/asset-prompts/_index.md` and display all templates with descriptions.
...
2. Write the template file to `_docs/asset-prompts/<template-name>.md`
3. Update `_docs/asset-prompts/_index.md`
...
On first invocation, if `_docs/asset-prompts/` doesn't exist:
...
>
> The image should include "The Forgotten Ways" in bold serif text,
