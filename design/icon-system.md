...
1. Check if the Icon wrapper component exists (e.g., `src/components/ui/icon.tsx`). Read it if so.
2. Read the global CSS file — note existing CSS custom properties.
3. Read the Tailwind config — note the spacing/sizing token scale.
4. Search for current icon import patterns: look for imports from the primary icon library across `src/`
5. Note any existing raw icon usage to understand migration scope.
...
Write `{{ICON_WRAPPER_PATH}}` (e.g., `src/components/ui/icon.tsx`):
...
/**
 * Canonical Icon component. Always use this instead of raw icon imports.
 *
...
**Note:** If the project uses a different icon library (e.g., Heroicons, Phosphor), adapt the `LucideIcon` type import accordingly.
...
- [ ] TypeScript types: icon type, `IconSize`, `IconProps`
- [ ] Exported: `Icon`, `ICON_SIZES`, `IconSize`

## Step 4 — Document the Usage Conventions

Output the following as the Icon System Reference. Save to `{{DOCS_PATH}}/design/ICON_SYSTEM.md` if it doesn't exist:
...
One library, one wrapper. All icons go through `<Icon>` from the canonical wrapper path. Never import and render icons directly in application components.

## Library

Use a single, consistent icon library across the entire project. Requirements:
- Consistent grid size and stroke weight
...
4. **Dark mode:** Use CSS custom properties inside SVG files so they respect the design token system. Avoid hardcoded hex colors.
...
| Mixing multiple icon libraries | Use one library only |
| `size={18}` raw pixel on icon | Use `size="sm"` (16) or `size="md"` (20) |

---

## Step 5 — Check for docs directory

If the docs directory doesn't exist, create it and save `ICON_SYSTEM.md` there. If the file already exists, update it with any gaps.
...
- Path: {{ICON_WRAPPER_PATH}}
- Issues: [any gaps found]

### Conventions Document
- Status: [Created / Already exists / Updated]
- Path: {{DOCS_PATH}}/design/ICON_SYSTEM.md

### Migration Scope
- Raw icon usages found: X (run /icon-audit to fix)
- Files affected: [list]
