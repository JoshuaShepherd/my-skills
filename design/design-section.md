...
1. Read `{{STYLES_PATH}}` to understand the current design tokens (CSS variables)
2. Read `{{CONFIG_PATH}}` to understand tenant theming
...
- Use `brandConfig` for any text that could vary per tenant
