---
name: asset-deliver
description: Package generated prompts or assets into a polished interactive HTML page with tabbed navigation, one-click copy buttons, and visual polish. Use to deliver prompt sets, asset galleries, or comparison views as a premium shareable artifact.
user-invocable: true
...
1. Read `src/lib/config/tenant.config.ts` for brand name (used in footer/branding)
2. Read `src/app/globals.css` for brand color tokens (if using brand theme)
...
<body>
  ...
  <div class="container">
    ...
    <div class="footer">
      Generated with Nano Banana 2 &mdash; {{BRAND_NAME}} &mdash; {{DATE}}
    </div>
  </div>
  ...
  <script>
    ...
    function fireConfetti(origin) {
      ...
      const particles = Array.from({ length: 40 }, () => ({
        ...
        color: ['#BFF549', '#D4A84B', '#C75B39', '#87A878', '#FAF5E4'][Math.floor(Math.random() * 5)],
        life: 1,
