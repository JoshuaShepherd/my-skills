...
Read `{{APP_ROOT}}/vercel.json`. If missing, flag as **CRITICAL** -- Vercel won't know how to route API calls.
...
Read the serverless entry file (typically `{{APP_ROOT}}/api/index.ts`). Verify:
...
Compare the dev server entry (e.g., `{{APP_ROOT}}/server/index.ts`) with the serverless entry (`{{APP_ROOT}}/api/index.ts`):
...
- **Client-side vars** (used in `src/`, must have `VITE_` prefix for Vite or `NEXT_PUBLIC_` prefix for Next.js)
...
- List all `VITE_*` or `NEXT_PUBLIC_*` vars -- these are baked into the JS bundle during build
- Check if vite.config.ts `define` block maps non-prefixed vars to client-side vars
...
Check if pre-build or type-check scripts will block the Vercel build:
...
```
GET {url}/api/protected-resource
```
Expected: 401 with `{"error":"Missing authorization token"}` or equivalent
