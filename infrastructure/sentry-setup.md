...
2. Read `{{APP_ROOT}}/src/main.tsx` to understand the React entry point and React version
3. Read `{{APP_ROOT}}/server/index.ts` and `{{APP_ROOT}}/server/vercel.ts` to understand Express setup
4. Read `{{APP_ROOT}}/vite.config.ts` to understand existing plugins and build config
5. Read `{{APP_ROOT}}/.env.example` to see existing env var patterns
6. Check if Sentry is already installed (`grep "@sentry" package.json`)
7. If the project has a type-safety guide, read it for type conventions
...
- `tracePropagationTargets` -- only propagate to `/api` (the Express backend via Vite proxy)
...
root.render(
  <StrictMode>
    {/* Wrap with your providers (QueryClientProvider, AuthProvider, etc.) */}
    <App />
  </StrictMode>
);
```
...
3. `pnpm dev` -- app starts without errors (Sentry gracefully disabled without DSN)
...
- **Sentry.ErrorBoundary** for granular UI sections with custom fallback UIs
...
- **Custom breadcrumbs** in API fetch utilities for request tracking
